...
class SafeBackupCLI {
  ...
  async run() {
    try {
      ...
      console.log(chalk.cyan('║   🔒 안전한 Google Sheets 백업 시스템      ║'));
      console.log(chalk.cyan('╚════════════════════════════════════════════╝\n'));
  ...
  async interactiveMode() {
    ...
    const { options } = await inquirer.prompt([
      {
        ...
        choices: [
          ...
          { name: '업로드 후 전체 검증', value: 'validate', checked: true },
      ...
      const advancedSettings = await inquirer.prompt([
        {
          ...
          message: '배치 크기 (1-500):',
          default: 100,
          validate: (value) => value >= 1 && value <= 500
        },
        {
          type: 'number',
          name: 'maxRetries',
    ...
    console.log(chalk.white(`   • 배치 크기: ${batchSize}`));
  ...
  async executeBackup(options) {
    ...
    try {
      ...
      // 설정 커스터마이징
      if (options.batch) {
        backupService.config.batchSize = options.batch;
      }
      if (options.retry) {
      ...
      if (options.validate !== undefined) {
        backupService.config.validateAfterBatch = options.validate;
      }
      ...
      if (error.message.includes('502') || error.message.includes('Server Error')) {
        ...
        console.log(chalk.yellow('\n💡 해결 방법:'));
        console.log(chalk.yellow('   1. 배치 크기를 줄여보세요 (예: --batch 50)'));
        console.log(chalk.yellow('   2. 잠시 후 다시 시도하세요'));
        console.log(chalk.yellow('   3. --resume 옵션으로 중단된 지점부터 재개하세요'));
        console.log(chalk.yellow('\n예시: node safe-backup-to-sheets.js --resume --batch 50'));
      } else {
        console.error(chalk.red('\n❌ 백업 실패:'), error.message);
      }
