...
class EnhancedResumeSubscriptionUseCase {
  ...
  async execute(profileId, options = {}) {
    ...
    stuckChecker = setInterval(() => {
      ...
      if (timeSinceProgress > MAX_STUCK_TIME && !result.timedOut) {
    ...
    try {
      ...
      // updateProgress 함수 전달
      await this.navigateToPremiumPage(browser, updateProgress);
   ...
   * Frame-safe 네비게이션 사용
   */
  async navigateToPremiumPage(browser, updateProgress = null) {
    // Enhanced Navigation Service 초기화
  ...
  async executeResumeWorkflow(browser) {
    ...
    try {
      ...
      // Case 1: 이미 활성 상태인 경우 (Pause 버튼 존재)
      if (statusBeforeAction.isActive && statusBeforeAction.hasPauseButton) {
        console.log(chalk.green('    ✅ 이미 활성 상태입니다 (Pause 버튼 존재)'));
        await saveDebugScreenshot('workflow-already-active');
        ...
        return {
          ...
          message: '이미 활성 상태',
          alreadyActive: true,
          statusText: statusBeforeAction.statusText,
          nextBillingDate: statusBeforeAction.nextBillingDate
        };
      }
      ...
      // Case 4: 활성 상태이지만 Pause 버튼이 없는 경우 (드문 경우)
      else if (statusBeforeAction.isActive && !statusBeforeAction.hasPauseButton) {
        console.log(chalk.yellow('    ⚠️ 활성 상태로 보이지만 Pause 버튼이 없음'));
