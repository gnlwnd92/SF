...
class TerminalLogger {
      ...
      return JSON.stringify(obj, (key, value) => {
        ...
        if (value instanceof Error) {
          return {
            ...
            stack: value.stack ? this.sanitizeMessage(value.stack) : undefined
          };
        }
  ...
  async captureLog(level, args) {
    try {
      ...
      if (process.env.DEBUG_MODE === 'true') {
        this.originalConsole?.error?.('로그 캡처 실패:', error.message);
      }
    }
  }
    ...
    try {
      ...
      try {
        ...
        const cleanContent = content
          .replace(/\\x1b\[[0-9;]*m/g, '') // ANSI 코드 제거
      ...
      // 로그 추가 전 정제
      const cleanedBufferCopy = bufferCopy.map(entry => ({
        ...entry,
        message: this.sanitizeMessage(entry.message || ''),
        raw: Array.isArray(entry.raw) ? entry.raw.map(item => 
          typeof item === 'string' ? this.sanitizeMessage(item) : item
        ) : entry.raw
      }));
      
      // 로그 추가
      data.logs.push(...cleanedBufferCopy);
      ...
      // 디버그 모드에서만 에러 출력
      if (process.env.DEBUG_MODE === 'true') {
        this.originalConsole?.error?.('로그 플러시 실패:', error.message);
      }
  ...
  flushBufferSync() {
    ...
    try {
      ...
      // 로그 추가 전 정제
      const cleanedBufferCopy = this.logBuffer.map(entry => ({
        ...entry,
        message: this.sanitizeMessage(entry.message || ''),
        raw: Array.isArray(entry.raw) ? entry.raw.map(item => 
          typeof item === 'string' ? this.sanitizeMessage(item) : item
  ...
  static getInstance(config) {
    if (!TerminalLogger.instance) {
      TerminalLogger.instance = new TerminalLogger(config);
    }
...
module.exports.getTerminalLogger = (config) => {
  return TerminalLogger.getInstance(config);
};

// 초기화 함수
module.exports.initializeTerminalLogger = async (config) => {
  const logger = TerminalLogger.getInstance(config);
