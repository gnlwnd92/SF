...
class UniversalStatusDetector {
  ...
  async analyzeExpandedUI(page) {
    ...
    const status = await page.evaluate(() => {
      ...
      // 다양한 날짜 형식 추출 (개선된 패턴)
      const datePatterns = [
        // 영어 월 이름 + 일 (+ 연도): Oct 7, 2025 또는 October 7
        new RegExp(`(${monthPattern}\\s+\\d{1,2}(?:,?\\s+\\d{4})?)`, 'gi'),
        // 일 + 영어 월 이름 (+ 연도): 7 Oct 2025 또는 7 October
        new RegExp(`(\\d{1,2}\\s+${monthPattern}(?:\\s+\\d{4})?)`, 'gi'),
        // 숫자 형식: 10/7/2025 또는 10/7
        /(\d{1,2}\/\d{1,2}(?:\/\d{2,4})?)/g,
        // ISO 형식: 2025-10-07
        /(\d{4}-\d{2}-\d{2})/g,
        // 한국어 형식: 1월 8일
