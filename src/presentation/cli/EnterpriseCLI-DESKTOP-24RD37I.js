...
class EnterpriseCLI {
  ...
  displayInitStatus() {
    ...
    console.log(chalk.gray('  • 스프레드시트 ID: ') + chalk.blue('1kHXW5JRPNBzrgv1Nkx-UBEaNMBNTTmcTCq-hvJzIw3k'));
  ...
  async listProfiles() {
    try {
      ...
      const sheetsRepository = new EnhancedGoogleSheetsRepository({
        spreadsheetId: '1kHXW5JRPNBzrgv1Nkx-UBEaNMBNTTmcTCq-hvJzIw3k'
      });
      const adsPowerAdapter = this.container.resolve('adsPowerAdapter');
  ...
  async resumeSubscriptionEnhanced() {
    ...
    try {
      ...
      const sheetsRepository = new SimpleGoogleSheetsRepository({
        spreadsheetId: this.config.googleSheetsId || '1kHXW5JRPNBzrgv1Nkx-UBEaNMBNTTmcTCq-hvJzIw3k'
      });
      ...
      for (let i = 0; i < selectedTasks.length; i++) {
        ...
        try {
          ...
          if (result.status === 'recaptcha_required') {
            ...
            // 🛡️ 활성 상태 오판단 방지: 한 번 더 재실행해서 검증
            if (result.status === 'already_active' && !task.hasRetried) {
              this.spinner.warn(`${task.googleId} - 활성 상태 감지됨`);
              console.log(chalk.yellow(`  ⚠️ 오판단 방지를 위해 브라우저를 재시작하고 한 번 더 확인합니다`));
              console.log(chalk.gray(`  → 다음 계정으로 넘어가기 전에 재검증 수행\n`));

              // 🔄 브라우저 완전히 종료 대기 (프로세스 정리)
              await new Promise(r => setTimeout(r, 3000));

              console.log(chalk.cyan(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`));
              console.log(chalk.cyan(`🔁 재검증: ${task.googleId}`));
              console.log(chalk.cyan(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`));

              try {
                this.spinner = ora(`재검증 중...`).start();

                // 같은 계정을 다시 실행 (hasRetried 플래그 추가)
                const retryTask = { ...task, hasRetried: true };

                const retryResult = await enhancedResumeUseCase.execute(actualAdsPowerId, {
                  googleId: retryTask.googleId,
                  email: retryTask.googleId,
                  rowIndex: retryTask.rowIndex,
                  profileData: {
                    email: retryTask.googleId,
                    password: retryTask.password,
                    recoveryEmail: retryTask.recoveryEmail,
                    code: retryTask.code,
                    totpSecret: retryTask.totpSecret || retryTask.code,
                    googleId: retryTask.googleId
                  }
                });

                // 재검증 결과 처리
                if (retryResult.status === 'already_active') {
                  // 여전히 활성 상태 → 진짜 활성 상태
                  this.spinner.succeed(`${task.googleId} - 활성 상태 확정`);
                  console.log(chalk.green(`  ✅ 재검증 완료: 정말로 활성 상태입니다`));
                ...
                } else if (retryResult.status === 'resumed') {
                  // 재검증에서 일시중지 감지 → 재개 성공!
                  this.spinner.succeed(`${task.googleId} - 재개 성공 (False Positive 방지)`);
                  console.log(chalk.green(`  🎯 초기 판단이 잘못되었습니다!`));
                  console.log(chalk.green(`  ✅ 재검증 후 일시중지 상태 확인 → 재개 완료`));
                  if (retryResult.nextBillingDate) {
                    console.log(chalk.green(`  ✔ 다음 결제일: ${retryResult.nextBillingDate}`));
                  }
                  console.log(chalk.green(`  ✔ 처리 시간: ${retryResult.duration}초`));
                  ...
                  // 기타 상태
                  this.spinner.info(`${task.googleId} - ${retryResult.status}`);
                  console.log(chalk.cyan(`  ℹ️ 재검증 결과: ${retryResult.status}`));
                  results.success.push(task.googleId);
                }

              } catch (retryError) {
                this.spinner.fail(`${task.googleId} - 재검증 실패`);
                console.log(chalk.yellow(`  ⚠️ 재검증 중 오류: ${retryError.message}`));
                console.log(chalk.yellow(`  → 초기 결과(활성 상태)를 유지합니다`));
                results.success.push(task.googleId);
              }
              ...
              // 일반적인 성공 케이스 (활성 상태가 아니거나 이미 재검증 완료)
            ...
            // 결제 수단 문제 특별 처리
            if (result.error === 'PAYMENT_METHOD_ISSUE' ||
